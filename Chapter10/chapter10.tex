...
%****************************** Third Chapter **********************************
%*******************************************************************************
\chapter{Sensitivity Predictions for Heavy Neutral Leptons at SBND}
\label{ChapterResult}
...
The beam bucket distribution with fully propagated uncertainties is the input of the limits setting, which exploits the exceptionally high signal-to-background ratio of bins at the edge of the distribution, as discussed in Chapter \ref{ChapterSelect}, to achieve competitive sensitivity.

The following chapter provides details of the analysis steps employing the three beam bucket distributions after the selection, including two acquired from the lenient and stringent selection performed on reconstructed variables and a third acquired by smearing true variables. 
...
The results of the upper limits on the coupling $|U_{\mu4}|^{2}$ of HNLs are presented in Section \ref{sec:result}.
...
Then, the weight $w$ describing whether the probability $P(x')$ is more or less likely to occur given the transformed parameter $x'$ compared to the CV parameter $x$ is computed as:
\begin{equation}
\label{eq:prob_w}
	w = \frac{P(x')}{P(x)}.
\end{equation}
...
For example, recombination, as detailed in Section \ref{sec7:delta}, influences not only the charge and light yield but also the non-uniformity of charge deposition on wires \cite{argoneut_recomb, icarus_recomb}. 
...
The impact of uncertainties can be evaluated for a distribution of a physics measurement, of which the distribution of interest is the beam bucket distribution describing the arrival time at the front face of SBND (See Section \ref{sec:key_dist}).
The reweighting method results in a series of observations $n$ of the beam bucket distribution, consisting of the CV observation $n^{CV}$ and observations $n^k$ from a total of $U$ universes.
...
Since there are multiple sources of uncertainties, the total covariance matrix is computed by summing the covariance matrix from each source together, effectively adding the uncertainties in quadrature as follows:
\begin{equation}
	V^{Total}_{ij} = \sum_{Sources} V_{ij}^{Source} = V_{ij}^{Stat} + V_{ij}^{Flux} +V_{ij}^{Cross\ Section} + ...
\end{equation}
The total uncertainty is then computed from the total covariance matrix $V_{ij}$ using Eq. \ref{eq:diag_cov}.
...
\end{equation}
which allows for easy and direct comparison across bins of signal and background samples.
...
The statistical uncertainty is evaluated using the number of signal slices selected, of which the following plots show the distribution after the lenient selection detailed in Chapter \ref{ChapterSelect}.
Meanwhile, the cosmic mistagging uncertainty is evaluated using the number of cosmic muon slices occurring in the same readout window of HNLs that remain after selection.
The impact of the uncertainties due to flux modelling can be assessed using the reweighting method.
The detector systematics however is not reweightable and requires a fully simulated and reconstructed MC sample for assessing uncertainties due to the detector as previously explained in Section \ref{sec:reweighting}.
...
Thus, detector systematics are not included in the limits setting presented here but should be included in future iterations of this analysis.
All of the discussed uncertainties were assessed using the beam bucket distributions that describe the arrival time at the front face of SBND (See Section \ref{sec:key_dist}).  

The statistical uncertainty indicates the statistical fluctuation of the MC sample.
...
\end{equation}
The uncertainty is computed before the normalisation to the Proton On Target (POT) exposure of 3 years of data taking.
...
\begin{figure}[htbp!]
	...
	Uncertainties of HNLs, including (a) statistical, (b) cosmic mistagging and (c) flux.
	}
        \label{fig:hnl_error}
	\vspace{0.5cm}
%\end{figure}
...
\caption[Total Uncertainty of Heavy Neutral Leptons]{
...
The flux prediction and the reweighting framework for assessing the uncertainties were both developed by the MiniBooNE experiment and furthered by the MicroBooNE experiment \cite{BNBFlux}.
The framework has now been implemented for SBND, called \texttt{SBN Event Weight} \cite{sbnweight_module} and intended for a consistent reweighting method across the experiments in the SBN program, including SBND, ICARUS and MicroBooNE.
...
\begin{coloritemize}
	...
	\item \textbf{Production}: The flux prediction for the number of secondary mesons produced in the BNB target has an uncertainty associated with each particle type (See Section \ref{sec4BNB}). 
...
The flux fractional uncertainty is plotted in the green line in the bottom panel of Fig. \ref{fig:hnl_total_error} that it is well-constrained $< 8\%$.
...
The total fractional uncertainty is shown in the purple line in the bottom panel of Fig. \ref{fig:hnl_total_error}, demonstrating that it is well-constrained $< 10\%$ across the full distribution.
...
The detector systematics uncertainty is currently not included but should be included in future work.
...
Similarly to HNLs, background uncertainties were assessed using the arrival time distributions (See Section \ref{sec:key_dist}).  
...
This is visually evident in the statistical fractional uncertainty plotted in the blue line in the bottom panel of Fig. \ref{fig:bkg_total_error}.
For bins at the centre of the beam bucket, the statistical fractional uncertainty is constrained at $< 20\%$.
For bins at the edge of the beam bucket distribution, particularly the first and last 4 bins, the statistical fractional uncertainty reaches the level about $100\%$.
...
Particularly, the Sanford-Wang parametrisation for modelling the $\pi^+$ production introduces biases for NC $\pi^0$ interactions \cite{EdPhD}, which is the main background contributor.
...
\begin{figure}[htbp!]
	...
	Uncertainties of SM neutrino and cosmic backgrounds, including (a) statistical, (b) flux and (c) SM neutrino cross section.
	}
        \label{fig:bkg_error}
	\vspace{0.5cm}
%\end{figure}
...
\caption[Total Uncertainty of SM Neutrinos and Cosmic Backgrounds]{
Total uncertainty (top) and fractional uncertainties (bottom) of SM neutrino and cosmic backgrounds.
}
\label{fig:bkg_total_error}
\end{figure}

The SM neutrino cross section uncertainty was assessed using the \texttt{SBN Event Weight} framework.
The systematics variables of models employed by the GENIE generator are provided in Ref. \cite{genie_tune} (See Section \ref{sec:gen_genie}).
...
Bins at the centre of the beam bucket distribution have the lowest cross section uncertainty in the entire distribution at $< 50\%$.
...
The total fractional uncertainty combining the statistical, flux and cross section uncertainty, is plotted in the purple line in the bottom panel of Fig. \ref{fig:bkg_total_error}.
...
The sensitivity of SBND to HNLs is determined as the upper limits on the coupling $|U_{\mu4}|^2$ of HNLs.
...
The limits setting procedure employs the likelihood-based hypothesis test \cite{asymptotic_test}, of which likelihood functions are constructed using the beam bucket distribution of signals and backgrounds.
...
Other parameters in the hypothesis characterising the shape of the distributions represent \textit{nuisance parameters}, denoted as $\theta$.
...
\end{equation}
The first product describes the likelihood of the distribution $n$ with bin $i$, which is constructed using the beam bucket distribution of HNLs as signals and SM neutrinos with cosmics as backgrounds as described in Eq. \ref{eq:expectation}. 
...
The setting limit procedure is performed fully using the \texttt{pyhf} package, with the test statistic described in the previous section.
The arrival time distributions are input into the \texttt{pyhf} package to infer the upper limit at C.L. of 90\%.
The histograms input to the background-only $H_{b}$ and the signal+background $H_{s+b}$ hypotheses are as following:
\begin{itemize}
...
As previously stated as the timing cut (See Section \ref{sec:select_result}), the relevant high signal-to-noise bins are the first and last 4 bins of the beam bucket distribution, which lead to the same result to be demonstrated later.

Three beam bucket distributions were used in the limits setting.
Appendix \ref{appendix_lenient} and \ref{appendix_stringent} contain the beam bucket distributions for signals and backgrounds after the lenient and stringent cut respectively (See Section \ref{sec:select_result}), with all uncertainties propagated (See Section \ref{sec:uncertainty}) . 
Additionally, appendix \ref{appendix_smeared} contains the smeared truth distributions under the assumption of timing reconstruction improvement (See Section \ref{sec:truth_bucket}).
...
was applied with a lenient selection as shown in Fig. \ref{fig:final_relaxed} and the other one was applied with a stringent selection as shown in Fig. \ref{fig:final_strict} (See Section \ref{sec:select_result}).
The third distribution is the smeared truth as shown in Fig. \ref{fig:final_truth}, which was constructed by smearing \textit{true} variables without any detection simulation and reconstruction applied (See Section \ref{sec:truth_bucket}).
...
Unlike the reconstructed distributions with full uncertainties propagated, the smeared truth distribution does not contain any uncertainties for simplicity. 
The first and last 4 bins of every distribution are also shown to demonstrate the significantly high signal-to-background ratio of these bins.
...
\begin{figure}[htbp!]
	    ...
	    \label{fig:final_truth}
     	    \vspace{0.5cm}
        \end{subfigure}
        \caption[Beam Bucket Distributions Used in the Limits Setting]{
...
The lenient distribution results in limits excluding the coupling $|U_{\mu4}|^2$ from $9.18 \times 10^{-7}$ to $1.35 \times 10^{-8}$ across the mass range from 140 to 260 MeV.
The stringent cut results in more competitive limits, pushing the exclusion region of the coupling further down from $5.37 \times 10^{-7}$ to $7.65 \times 10^{-9}$ at the same mass range.
...
As can be observed from bins at the edge of the beam bucket distribution shown in Fig. \ref{fig:final_strict}, the limited background statistics might be insufficient to describe the underlying distribution, which can lead to large statistical fluctuations.                                  
...
\begin{figure}[hb!]
    ...
    \caption[Expected Upper Limits On The Coupling $|U_{\mu4}|^2$ for Majorana HNLs]{Expected upper limits and the Brazil bands on the coupling $|U_{\mu4}|^2$ for Majorana HNLs, for the lenient (left) and stringent distribution (right).}
    \label{fig:nupi0_reco_result}
\vspace{0.5cm}
\end{figure}

Fig. \ref{fig:nupi0_reco_full_edge} shows the comparison between limits setting using the entire histogram and only the first and last 4 bins, which were previously referred to as the timing cut in Chapter \ref{ChapterSelect}. 
The upper limits are the same for both cases.
...
\begin{figure}[ht!]
    ...
    \caption[Comparison Between Limits Setting Using The Entire Distibution and Only Edge Bins]{Comparison between limits setting using the entire histogram and only the edge bins of the lenient (left) and stringent distribution (right).}
    \label{fig:nupi0_reco_full_edge}
\end{figure}

Fig. \ref{fig:nupi0_reco_truth} shows the upper limits from all three beam bucket distributions.
...
As detailed in Section \ref{sec:truth_bucket}, the smeared truth distribution was produced under the assumption of a better timing reconstruction to explore its impact on sensitivity. 
...
\begin{figure}[ht!]
    ...
    \caption[Upper Limits Comparison Between The Lenient, Stringent and Smeared Truth Distribution]{Comparison between limits resulting from the lenient and stringent reconstructed distributions and the smeared truth distribution.}
...
\begin{table}[htb!]
\caption{Table summarising expected upper limits results.}
\label{table:result}
\begin{center}
...
\begin{figure}[htb!]
    ...
    \caption[Comparison Between SBND Expected Upper Limits and Existing Results]{
    Upper limits on the coupling $|U_{\mu4}|^{2}$ at the 90\% confidence level for Majoranan HNLs in the mass range of $0 < m_{N} < 265$ MeV.
SBND MC results is compared with existing experimental results, including SIN \cite{SIN3}, PIENU \cite{PIENU}, MicroBooNE \cite{uboone1, uboone2, uboone3}, KEK E89 \cite{KEK2}, KEK E89/E104 \cite{KEK3}, PS191 \cite{PS191C}, E949 \cite{E949}, NA62 \cite{NA62B}, NuTeV \cite{NuTeV} and T2K \cite{t2k}.
}
\label{fig:sensitivity}
\end{figure}

The limits acquired from the smeared truth distribution, as shown by the green line, is the most competitive out of the three results presented here.
...
Three upper limits on the coupling $|U_{\mu4}|^2$ of Majorana HNLs are presented in this chapter to demonstrate the range of the physics capability of SBND.
...
The limits from the smeared truth distribution are the most competitive that can probprobe region not yet explored by other experiments.
This limit also demonstrates the potential that SBND can achieve if the timing reconstruction is improved with a better resolution.
