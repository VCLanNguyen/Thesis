...
%****************************** Third Chapter **********************************
%*******************************************************************************
\chapter{Setting Upper Limits on The Mixing Angle of HNL}
\label{ChapterResult}
...
%The second product describes the likelihood of an auxiliary histogram $\boldsymbol{m}$ with bin index $k$, representing measurement which constrains the uncertainties.
...
%	&   \multirow{2}{=}{\multicolumn{1}{c}{Uncertainty}} & \multicolumn{1}{c|}{Modifier}    & \multicolumn{1}{c|}{MC Sample}    & \multicolumn{1}{c|}{Bin}       \\ %\hline{3-6}
%&   &  \multicolumn{1}{c|}{Shape}    & \multicolumn{1}{c|}{Correlation}    & \multicolumn{1}{c|}{Correlation}        \\ %\cline{3-6}
\hline &&&&\\[0.1cm]
...
Moreover, Fig. \ref{fig:nupi0_reco_full_edge} shows the comparison between limit setting using the entire beam bucket distribution and only the first and last 4 bins, which were previously referred to as the timing cut in Chapter \ref{ChapterSelect}. 
