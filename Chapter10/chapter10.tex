...
The impact of uncertainties can be evaluated for a distribution of a physics measurement, of which the distribution of interest is the one describing the arrival time at the front face of SBND (See Section \ref{sec:key_dist}).
...
Uncertainties discussed here were assessed using the arrival time distributions that describe the arrival time at the front face of SBND (See Section \ref{sec:key_dist}).
...
\begin{table}[htbp!]
\caption[Summary of Uncertainty Constraints]{Table summarising modifiers used to constrain uncertainties.}
...
\begin{figure}[htbp!]
	...
	HNLs are normalised to $m_N = 200$ MeV and $|U_{\mu4}|^2 = 3.16 \times 10{-7}$.
	}
\end{figure}
...
\begin{table}[htbp!]
	\caption[Summary of Expected Limits on the Coupling $|U_{\mu4}|^2$ of Majorana HNLs]{Table summarising expected upper limits on the coupling $|U_{\mu4}|^2$ of Majorana HNLs at the C.L. of 90\% predicted for SBND.}
\label{table:result}
\begin{center}
