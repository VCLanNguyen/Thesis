...
These measurements will be extremely beneficial for advancing the objectives of the SBN physics program, as well as contributing to the physics goals of the Deep Underground Neutrino Experiment (DUNE), which employs argon as its detector medium.
...
Each window, measuring 60 cm $\times$ 50 cm, houses a fibreglass plate laminated on both sides with non-conductive reflective foils $> 99\%$ specular reflection in the visible range and coated with TetraPhenyl Butadiene (TPB).
...
Such difference can be exploited to perform the timing reconstruction using only PMT signals, which will be further discussed in Section \ref{sec:reco_pds}.
...
This configuration enables a comprehensive strategy for cosmic background rejection utilising both geometrical and high precision timing information, which will be more detailed in Section \ref{sec:crumbs}.
...
%CAEN digitizer
Regarding the electronics readouts of the PDS, CAEN digitizers are employed. % depicted as the blue boxes in Fig. \ref{fig:daqOverview}
...
The characterisation of synchronisation across multiple V1730SB digitizers is detailed in Section \ref{sec4PMT}.
...
The module can provide a bias voltage, which is adjustable for individual SiPMs, as well as signal amplification and shaping.
...
These sets of tracks are useful for electron lifetime measurements, which will be explored in Section \ref{sec7:etime}.
...
The data is then assemble to build a physics event, of which the DAQ process will be explained in Section \ref{sec4DAQOverview} and \ref{sec:evb} next.
...
The signals are acquired by readout electronics, shown as colour boxes in the first column labelled \textit{Readout Electronics}, that was covered earlier in Section \ref{sec:readout}.
...
The event building process will be discussed next in Section \ref{sec:evb}.
...
The timing resolution of the CRT readouts is $\mathcal{O}$ (1 ns), which will be demonstrated later in Section \ref{sec4InternalClock}.
...
Their intensity is measured by two steroids, while their positioning and timing are monitored by beam position monitors and Resistive Wall Monitors (RWM) \cite{BNBRWM}.
...
The SBND experiment, together with other experiments in the SBN program, aims to conclusively address the low energy excess observed across experiments such as LSND, MiniBooNE, and other nuclear reactor and solar neutrino experiments. 
