...
%The reconstructed beam bucket for SM neutrinos resembles a Gaussian with a width of $2.26$ ns as compared to the proton bucket from the Booster synchrotron with an intrinsic width of $1.308$ ns.
...
The number of hits is particularly important given that Pandora relies on hit information to reconstruct 3D information of particles in a slice (See Sections \ref{sec:hit_finding} and \ref{sec:pandora}).
...
The beam bucket distribution after the cut is shown in Fig. \ref{fig:bb_postNhits}, where it can be seen that the cut reduces the signal and background efficiency minimally by < 1\%.
...
For a matched flash, the fraction describes the level of agreement between $L_{\mathrm{Q}}$ and $L$, where $L_\mathrm{Q}$ is the number of PhotoElectrons (PEs) predicted from the reconstructed charged and $L$ is the number of PEs measured by PMTs.
...
The peak region of the bucket is dominated by the primary background from NC $\pi^0$ interactions which have proven to be a very tricky background to remove due to their similarity with HNL showers.
A combination of CC $\nu_\mu$ and Other NC remain even though they were not considered to be a background at the beginning.
...
In addition, some backgrounds from Non-FV and dirt neutrino interactions can still be seen, since their products can propagate to the FV and deposit energy.  
Finally, cosmic muons are almost fully rejected is due to aggressive cuts. 
...
To better understand the sensitivity dependency on the signal-to-background ratio, two selection procedures were developed.
...
%The resulting beam bucket corresponds to a Gaussian with a width smeared from 1.308 ns to 2.26 ns.
...
\begin{figure}[hb!]
    ...
    \caption{The beam bucket distribution of SM neutrinos in the truth phase space (left) and in the reconstruction phase space (right).}
...
Unlike the background modelling approach of normalising the same area, an efficiency of 30\% was applied to the HNL truth beam bucket distribution to account for the combined effects of reconstruction and selection.
...
All three beam bucket distributions, from both the lenient and stringent cut on reconstructed variables and from the smeared truth variables, are used for studying the sensitivity to HNLs in Chapter \ref{ChapterResult} next. 
